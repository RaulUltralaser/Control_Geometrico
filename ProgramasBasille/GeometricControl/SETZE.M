function [B,C,D] = setze(A,B,C,D,P)
% SETZE  Set zeros.
%	 [B,C,D] = SETZE(A,B,C,D) or [B,C,D] = SETZE(A,B,C,D,P) . If called
%	 with D = O it sets C and D (if nb <= mc) or B and D (if nb > mc)
%	 such that the invariant zeros of the system
%		.
%		x = Ax + Bu
%		y = Cx + Du
%
...
nargs = nargin;
if (nargs ~= 4)&(nargs ~= 5)
  error('   wrong number of arguments in SETZE')
end
[t,na] = size(A);
[t,nb] = size(B);
[mc,t] = size(C);
